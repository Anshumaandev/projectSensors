...
export default function App() {
  ...
  const pprevTs = useRef(-1);
  const prevAccelVal = useRef(0);
  const prevPosVal = useRef(0);
  const pprevPosVal = useRef(0);
  const WestimatePosition = (accelVal, time) => {
    ...
    if (time === prevTs || time === pprevTs) {
      ...
      prevPosVal.current = pos;
      pprevPosVal.current = prevPosVal;
      pprevTs.current = prevTs;
      prevTs.current = time;
      prevVelocityVal.current = vel;
      prevAccelVal.current = accelVal;
  ...
  //**-------- NEW Initializations for Depth calculation---------  **// 
  const Direction = { UNKNOWN: 'UNKNOWN', UP: 'UP', DOWN: 'DOWN', DRAG: 'DRAG' };

  const usePumpInfo = (
    startPos, endPos, prevStartPos, prevEndPos,
    startTs, endTs, prevStartTs, prevEndTs,
    begin_pos, lowest_pos, end_pos,
    begin_ts, lowest_point_ts, end_ts
  ) => {
    const [pumpWindow, setPumpWindow] = useState([]);

    const getDepth = () => Math.abs(endPos - startPos);
    const getRecoil = () => prevEndPos - startPos;
    const getStartTs = () => startTs;
    const getEndTs = () => endTs;
    const getEndPos = () => endPos;
    const getStartPos = () => startPos;
    const getPumpWindow = () => pumpWindow;
    const setPumpWindow = (pw) => setPumpWindow(pw);
    ...
    return {
      ...
      getPumpWindow,
      setPumpWindow,
      // ... return other methods as needed ...
    };
  };

  const usePumpParser = (minDepth, maxDepth) => {
    const [positions, setPositions] = useState([]);
    const [dir, setDir] = useState(Direction.UNKNOWN);
    const [prevMin, setPrevMin] = useState(Number.MAX_SAFE_INTEGER);
    const [prevMax, setPrevMax] = useState(Number.MIN_SAFE_INTEGER);
    const [pPrevMax, setPPrevMax] = useState(Number.MIN_SAFE_INTEGER);
    const [prevMinTs, setPrevMinTs] = useState(0);
    const [prevMaxTs, setPrevMaxTs] = useState(0);
    const [pPrevMaxTs, setPPrevMaxTs] = useState(0);
    const [prev, setPrev] = useState(0);
    const [prevTs, setPrevTs] = useState(0);


    const parseNext = (position, velocity, acceleration, timestamp) => {
      let prevStartPos;
      let prevEndPos;
      let prevStartTs, prevEndTs;
      pumpWindow.current.push(new PosPoint(timestamp, position, velocity, acceleration));
      if (positions.current.length < 1) {
        dir.current = Direction.UNKNOWN;
        prevMin.current = position;
        prevMax.current = position;
        pPrevMax.current = position;
        pPrevMaxTs.current = timestamp;
        prev.current = position;
        positions.current.push(position);
        return null;
      }
      if ((position < prev.current)) {
        if (pPrevMax.current < prev.current) {
          // pPrevMax.current = prev.current;
          // pPrevMaxTs.current =timestamp;
        }
        if (dir.current === Direction.UP) {
          dir.current = Direction.DRAG;
          prevMax.current = prev.current;
          prevMaxTs.current = prevTs.current;
          end_pos.current = position;
          end_ts.current = timestamp;
        } else if (dir.current === Direction.DRAG) {
          if (localMaximaDirDrag.current >= PumpParser.MIN_DR_DRAG) {
            localMaximaDirDrag.current = 0;
            let depth = prevMax.current - prevMin.current;
            dir.current = Direction.DOWN;
            if (isValidPump(depth, positions.current.length - 1)) {
              if (prevPump.current === NULL_PUMP) {
                prevStartPos = prevMax.current;
                prevEndPos = prevMin.current;
                prevStartTs = pPrevMaxTs.current;
                prevEndTs = prevMinTs.current;
              } else {
                prevStartPos = prevPump.current.getStartPos();
                prevEndPos = prevPump.current.getEndPos();
                prevStartTs = prevPump.current.getStartTs();
                prevEndTs = prevPump.current.getEndTs();
              }
              let pumpInfo = new PumpInfo(prevMax.current, prevMin.current, prevStartPos, prevEndPos, pPrevMaxTs.current, prevMinTs.current, prevStartTs, prevEndTs, begin_pos.current, lowest_pos.current, end_pos.current, begin_ts.current, lowest_point_ts.current, end_ts.current);
              setPumpValues([pumpInfo.getStartPos(), pumpInfo.getEndPos(), pumpInfo.getDepth()]);
              pumpWindow.current = [];
              prevPump.current = pumpInfo;
              pPrevMax.current = prevMax.current;
              pPrevMaxTs.current = prevMaxTs.current;
              begin_pos.current = position;
              begin_ts.current = timestamp;
            }
            prevMin.current = Number.MAX_SAFE_INTEGER;
          } else {
            localMaximaDirDrag.current++;
          }
        } else {
          dir.current = Direction.DOWN;
        }
      } else {
        if (dir.current === Direction.DOWN || dir.current === Direction.DRAG) {
          if (prevMin.current > prev.current) {
            prevMin.current = prev.current;
            prevMinTs.current = prevTs.current;
            lowest_point_ts.current = timestamp;
    ...
    return {
      ...
      prevTs,
      parseNext,
      // ... return other state variables and setter functions as needed ...
    };
  };
